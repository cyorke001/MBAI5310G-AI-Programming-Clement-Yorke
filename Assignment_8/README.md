...
> **Note:** 100% accuracy is expected for a synthetic, template-based dataset with consistent and distinctive vocabulary per category. This result should not be interpreted as a guarantee of real-world performance. Realistic accuracy on real customer support messages would be in the range of **75–90%**, subject to data quality and volume.
...
**Priority metric — Recall:** For this use case, recall is more critical than precision. A missed classification (ticket routed to the wrong team) delays resolution and risks customer churn, particularly for high-priority categories such as Billing and Account_Access. A false alarm (incorrect team receives the ticket) is correctable with lower cost.
...
- **Channel bias:** Customers using Call Center may use different language patterns than those submitting typed messages, leading to uneven performance across channels.
